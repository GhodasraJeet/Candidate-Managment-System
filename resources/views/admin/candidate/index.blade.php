...
@endif
    ...
    @endcomponent
    @endif
    <h2 class="mt-5">Candidate Details</h2>
    ...
    <div class="row">
        <div class="table-responsive">
            <table class="table table-striped table-hover" id="candidateDetails">
                <thead>
                    <tr>
                        <th>Name</th>
                        <th>HR</th>
...
$(document).ready(function(){
    ...
    function fetchCategory(category='',from_date = '', to_date = '')
    {
        ...
        "responsive":true,
        ajax:{
            url:"{{route('admincandidate.index')}}",
            data: {category:category,from_date:from_date, to_date:to_date},
        },
        columns:[
            {data:"name",className:"name"},
            {data:"getHrDetails",className:"getHrDetails.name"},
...
//  Delete Candidate
    $('body').on('click', '.deleteCandidate', function () {
        ...
        if(confirm("Are You sure want to delete !"))
        {
            $.ajax({
            type: "DELETE",
            url: "{{ route('admincandidate.index') }}"+'/'+product_id,
